...
TITLE = "SimplyBigNews"
SHORT_DESC = "Clear, calm news in simple everyday words. Honest, jargon-free & easy to read."
assert len(TITLE) <= 30, f"Title too long: {len(TITLE)}"
assert len(SHORT_DESC) <= 80, f"Short desc too long: {len(SHORT_DESC)}"

FULL_DESC = """SimplyBigNews brings you the day's biggest headlines without media jargon, sensory overload, or sensationalism. Every story is carefully rewritten in plain, everyday words so you can stay informed in just two minutes.
...
• What Happened: Exactly 3 key bullet points—clean, fast, and no fluff.
• Why It Matters: How the news impacts your wallet, retirement, family, healthcare, and daily life.
• Word Helper: Tap difficult financial or political buzzwords for instant, plain-English definitions.
...
RELEASE_NOTES = """Initial launch of SimplyBigNews: Clear daily news rewritten in simple everyday words with listen-aloud audio narration and large-print comfort controls."""
assert len(RELEASE_NOTES) <= 500, f"Release notes too long: {len(RELEASE_NOTES)}"

def main():
    print("=" * 60)
    print(" Updating Google Play Store Listing to SimplyBigNews")
    print("=" * 60)
    ...
    try:
        ...
        # 2. Update production release notes
        ...
        print("\n" + "=" * 60)
        print("🎉 SUCCESS! Google Play Console has been updated to SimplyBigNews!")
